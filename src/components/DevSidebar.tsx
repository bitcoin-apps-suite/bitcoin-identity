...
const DevSidebar: React.FC<DevSidebarProps> = ({ onCollapsedChange }) => {
  ...
  useEffect(() => {
    // Fetch GitHub issue count - replace with your repository
    fetch('https://api.github.com/repos/bitcoin-apps-suite/bitcoin-identity/issues?state=open')
      .then(res => res.json())
  ...
  const menuItems: Array<{
    ...
    { path: '/token', icon: DollarSign, label: '$BID', badge: 'NEW' },
    { divider: true },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-identity', icon: GitBranch, label: 'GitHub', external: true },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-identity/issues', icon: Bug, label: 'Issues', external: true, badge: issueCount > 0 ? String(issueCount) : undefined },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-identity/pulls', icon: Code, label: 'Pull Requests', external: true },
    { divider: true },
